...
<s3xml>
    ...
    def testTransformParamterSolrAvailable(self):
        """ 
            Testing the Full-Text functionality in select()
            when transform paramter is set and solr available 
        """
        ...
        s3db = current.s3db
        q = ((S3FieldSelector("~.name").text("test")) | \
                (S3FieldSelector("~.document.file").text("test"))) | \
            ((S3FieldSelector("~.name").text("document")) | \
            (S3FieldSelector("~.document.file").text("document")))
        ...
        if ft:
            self.assertEqual(str(len(rows)), str(3))
        else:    
            self.assertEqual(str(len(rows)), str(0))

        query = resource.rfilter.get_query()
        if ft:
            a1, a2, a3 = re.search(r"(\d+,\d+,\d+)", str(query)).group().split(",")
            self.assertEqual(set([a1, a2, a3]), set([str(id1), str(id2), str(id3)]))

            pdoc = ["'PROJDOC1'", "'PROJDOC2'", "'PROJDOC3'"]
            pdoc1, pdoc2, pdoc3 = re.search(r"('\w+','\w+','\w+')", str(query)).group().split(",")
            self.assertEqual(set([pdoc1, pdoc2, pdoc3]), set(pdoc))
    ...
    # -------------------------------------------------------------------------
    def testTransformParamterSolrUnAvailable(self):
        """ 
            Testing the Full-Text functionality in select() 
            when transform paramter is set and solr Unavailable
        """
        import re
    ...
    def testFullTextFunctionTestingCallOutput(self):
        ...
        q = (S3FieldSelector("~.name").text("document")) | \
                (S3FieldSelector("~.document.file").text("document"))
        ...
        if ft:    
            ...
            self.assertEqual(str(expected), "False")
    ...
    def testFullTextFunctionSolrAvailable(self):
        ...
        if ft:
            ...
            a = re.search(r"\[\d+, \d+, \d+\]", str(query)).group()
            a = (a[1:])[:-1]
            a1, a2, a3 = a.split(", ")
            self.assertEqual(set([a1, a2, a3]), set([str(id1), str(id2), str(id3)]))
        else:
